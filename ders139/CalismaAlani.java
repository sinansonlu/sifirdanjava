...
public class CalismaAlani extends JPanel implements MouseListener, MouseMotionListener, ActionListener {
    ...
    public CalismaAlani(int boyutX, int boyutY) {
        ...
        tekilResim = new BufferedImage(boyutX, boyutY, BufferedImage.TYPE_4BYTE_ABGR);
    ...
    public void fareIslemiDolgu(MouseEvent e) {
        if(!animasyonVar) {
            ...
            if(e.getX() >= paletBaslangicX()
                ...
                int ustuneBasilanRenk = tekilResim.getRGB(x, y);
                dolguBoya(x, y, eldekiRenk, ustuneBasilanRenk);
                repaint();
            }
        }
    }

    public void dolguBoya(int x, int y, int renk, int ustuneBasilanRenk) {
        if(renk != ustuneBasilanRenk) {
            if(0 <= x && 0 <= y && x < tekilResim.getWidth() && y < tekilResim.getHeight()) {   
                int mevcutYerinRengi = tekilResim.getRGB(x, y);
                if(mevcutYerinRengi == ustuneBasilanRenk) {
                    tekilResim.setRGB(x, y, renk);
                    dolguBoya(x-1,y,renk,ustuneBasilanRenk);
                    dolguBoya(x+1,y,renk,ustuneBasilanRenk);
                    dolguBoya(x,y-1,renk,ustuneBasilanRenk);
                    dolguBoya(x,y+1,renk,ustuneBasilanRenk);
                }
            }
        }
    ...
    public ArrayList<BufferedImage> resimleriVer() {
        return resimler;
    }

    public void SpriteStackiAl(SpriteStack ss) {
        this.ss = ss;
    }
